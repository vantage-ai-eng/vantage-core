#!/usr/bin/env python3
"""Generate a busy still-ship Center simulation (local decisions/ + ingest plan).

Usage (from vantage-core/):
  python3 scripts/sim_center_busy.py [--out DIR]
  vantage-core center --suite … --decisions DIR --html DIR/center.html --open
"""
...
def _decision(
    *,
    ...
    obj = {
        ...
        "runner": {"name": "vantage-core", "version": "0.1.13"},
        "contract": {
...
def build_sim(out_dir: Path) -> None:
    ...
    samples = ROOT / "vantage_core" / "samples"
    paths_block = []
    for cid, fname, label in PATHS:
        ...
        target = sample if sample.is_file() else (ROOT / "examples" / "contracts" / "starters" / fname)
        paths_block.append(
            f"  - path: {target}\n"
    ...
    if not refuse.is_file():
        refuse = ROOT / "examples" / "contracts" / "starters" / "01_refuse_pii.yaml"
